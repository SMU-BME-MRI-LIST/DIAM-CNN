    ...
    while 1:
        ...
        if pd == 1:
            for i_case in range(int(batch_size)):
                filepath = '/public/siwenbin/DLQSM_YH/Aug_test/Demo12/data/patch/train/p'+str(i_Train)+'.h5'
                i_Train = i_Train + 1
            ...
            in_y = np.concatenate(patches_QSMs, axis=0)[..., np.newaxis]
             ...
             for i_case in range(int(batch_size)):
                 filepath = '/public/siwenbin/DLQSM_YH/Aug_test/Demo12/data/patch/valid/p'+str(i_Valid)+'.h5'
                 i_Valid = i_Valid + 1
                 f = h5py.File(filepath,'r')
                 f.keys()                       
                 patch_RDF = f['p_RDF'][:]            
...
if __name__ == '__main__':
    ...
    model, model_single = generate_multi_model(1, PATCH_SIZE, use_bn = True) # (number of GPU = 1)
    model.compile(loss= three_loss, optimizer = Adam(lr = 1e-3), metrics=[l1_loss, gradient_loss, dipole_loss])#
    #model.load_model('/public/siwenbin/DLQSM_YH/Aug_test/Demo1/tmp_Model/net_tmp1.h5') ## load model weights
    ...
    DIR_WEIGHT ='/public/siwenbin/DLQSM_YH/Aug_test/Demo12/tmp_model/' 
    ...
    DIR_LOG = '/public/siwenbin/DLQSM_YH/Aug_test/Demo12/log/test_log'
...
#        model = load_model(os.path.join(model_dir,'model_%03d.h5'%initial_epoch),custom_objects={'MirrorPadding3D':MirrorPadding3D,'three_loss':three_loss})
    ...
    model.save_weights('/public/siwenbin/DLQSM_YH/Aug_test/Demo12/Model/Model_QSM1_7_Demo12.h5')
