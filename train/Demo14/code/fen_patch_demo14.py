...
#
# Description :
# Patching training data for DIAM-CNN
#
# Input : 
...
for i_x in x:
    for i_y in y:
        for i_z in z:
            ...
            filename = '{0}/{1}/RDF.mat'.format(FD_DATA, i_case)#filename=FD_DATA£¬i_case£¬/RDF.mat'
            ...
            filename = '{0}/{1}/mask.mat'.format(FD_DATA, i_case)
            mask = np.real(load_mat(filename, varname='mask'))
            
            dipole_size = RDF.shape
            ...
            RDF_aug1 = augment_data(rdf_d1, voxel_size = VOXEL_SIZE, flip='', thetas=[-15, 15])
            RDF_aug2 = augment_data(rdf_d2, voxel_size = VOXEL_SIZE, flip='', thetas=[-15, 15])
            del QSM,RDF
               
            i_aug = i_z-1
            patches_RDF = extract_patches(RDF_aug[i_aug, ...], PATCH_SIZE, EXTRACTION_STEP)
            patches_RDF1 = extract_patches(RDF_aug1[i_aug, ...], PATCH_SIZE, EXTRACTION_STEP)
            patches_RDF2 = extract_patches(RDF_aug2[i_aug, ...], PATCH_SIZE, EXTRACTION_STEP)
            ...
            patches_RDF1 = patches_RDF1[idxs_valid, ...]
            patches_RDF2 = patches_RDF2[idxs_valid, ...]
            ...
            for i_case in CASES_TRAIN:
                filename = '/public/siwenbin/DLQSM_YH/Aug_test/Demo14/data/patch/valid/RDF/p'+str(i_case)+'.h5'
                ...
                filename = '/public/siwenbin/DLQSM_YH/Aug_test/Demo14/data/patch/valid/RDF1/p'+str(i_case)+'.h5'
                f = h5py.File(filename,'w') 
                f['p_RDF'] = patches_RDF1[(i_case-i_Total_pre),...] 
                f.close()
                
                filename = '/public/siwenbin/DLQSM_YH/Aug_test/Demo14/data/patch/valid/RDF2/p'+str(i_case)+'.h5'        
                f = h5py.File(filename,'w') 
                f['p_RDF'] = patches_RDF2[(i_case-i_Total_pre),...] 
                f.close()
