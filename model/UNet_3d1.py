...
def UNet_3d1(img_size, filter_base=32,
                      kernel_size=(3,3,3),
                      level=5,
                      use_bn=True,
                      use_deconv=True, 
    ...
    if multi_input:
        inputs = Input(img_size + (6,), name='input')
        x = Lambda(lambda x: x[..., 0:5])(inputs)
        x_ = Lambda(lambda x: x[..., 5:6])(inputs)
    ...
    for i in range(level-2, -1, -1):
        ...
        # Up-sampling
        if use_deconv:
            x1 = Conv3DTranspose(filter_size, kernel_size, strides=2, padding='valid', kernel_initializer='he_normal', name=name_template.format('deconv'))(x1)
